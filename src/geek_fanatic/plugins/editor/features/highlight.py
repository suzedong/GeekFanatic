...
class SyntaxHighlight(EditorFeature):
    ...
    def _initialize_default_tokens(self) -> None:
        ...
        self.register_token_type(
            TokenType(
                "keyword",
                r"\b(def|class|if|else|for|while|return|import|from|in|is|not|and|or|True|False|None|try|except|finally|raise|with|as|assert|break|continue|global|lambda|pass|yield)\b",
                "#0000FF",
            )
        )
        ...
        self.register_token_type(
            TokenType(
                "builtin",
                r"\b(print|len|str|int|float|list|dict|set|tuple|range|enumerate|zip|map|filter|sorted|any|all|sum|min|max|abs|round|type|isinstance|hasattr|getattr|setattr|delattr)\b",
                "#800080",
            )
        )
